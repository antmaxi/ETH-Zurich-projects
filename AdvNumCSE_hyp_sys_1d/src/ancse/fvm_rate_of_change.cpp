    ...
    const std::shared_ptr<SimulationTime> &simulation_time)
{
    ...
    if (config["reconstruction_variable"] == "primitive") {
        REGISTER_RECONSTRUCTION("minmod", PrimPWLinearReconstruction(MinMod{}, model))
        REGISTER_RECONSTRUCTION("monotonized_central",
                                PrimPWLinearReconstruction(MonotonizedCentral{}, model))
        REGISTER_RECONSTRUCTION("super_bee", PrimPWLinearReconstruction(SuperBee{}, model))
    } else if (config["reconstruction_variable"] == "conservative") {
        REGISTER_RECONSTRUCTION("minmod", ConsPWLinearReconstruction{MinMod{}})
        REGISTER_RECONSTRUCTION("monotonized_central",
                                ConsPWLinearReconstruction{MonotonizedCentral{}})
        REGISTER_RECONSTRUCTION("super_bee", ConsPWLinearReconstruction{SuperBee{}})
    }
