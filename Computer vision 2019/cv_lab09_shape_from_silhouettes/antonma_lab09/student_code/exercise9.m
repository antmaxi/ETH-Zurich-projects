...
%
loadImages           = 1;  % also displays silhouettes
displayVolumeCorners = 1;
computeVisualHull    = 1;
displayVolumeSlices  = 0;
displayIsoSurface    = 1;
...
% interest.
bbox = [0 0 -2; 3 3 2.5]; % [minX minY minZ; maxX maxY maxZ];
volumeX = 64;
volumeY = 64;
volumeZ = 128;
volumeThreshold = 17;
...
if computeVisualHull
    ...
    %     (x,y,z), but matrix element access in Matlab is mat(row,col).
        for x = 1:volumeX
            for y = 1:volumeY
                for z = 1:volumeZ
                    voxel_center = [x - 0.5, y - 0.5, z - 0.5, 1]';
                    voxel_center_glob = T*voxel_center;
                    for n=1:numCameras
                        voxel_center_xy = Ps{n}*voxel_center_glob;
                        voxel_center_xy = voxel_center_xy./repmat(voxel_center_xy(3,:),3,1);

                        fl_1 = round(voxel_center_xy(1));
                        fl_2 = round(voxel_center_xy(2));
                        if (fl_2 >= 1 && fl_2 <= size(sils{n},1))
                            if (fl_1 >= 1 && fl_1 <= size(sils{n},2))
    %                             figure(3);
    %                             hold off;
    %                             imshow(double(rgb2gray(ims{n}))/255.*sils{n});
    %                             hold on;
    %                             plot(5,5,'r*');
    %                             plot(300,5,'b*');
    %                             plot(voxel_center_xy(1,:),voxel_center_xy(2,:),'g*');
                                if sils{n}(fl_2, fl_1)
                                    volume(x,y,z) = volume(x,y,z) + 1;
                                    %plot(voxel_center_xy(1,:),voxel_center_xy(2,:),'r*');
                                %else
                               %     plot(voxel_center_xy(1,:),voxel_center_xy(2,:),'g*');
                                end
                                %drawnow;
...
if displayIsoSurface
    for volumeThreshold = 14:18
        % display result
        figure(volumeThreshold);
        clf;
        grid on;
        xlabel('x');
        ylabel('y');
